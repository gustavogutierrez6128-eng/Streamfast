...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [correo, passwordHash, 'basico', nombre, fechaRegistro]
    );
    res.json({ ok: true, message: 'Usuario registrado con éxito' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ ok: false, message: 'El correo ya está registrado o hubo un error interno' });
  }
});
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) return res.status(401).json({ ok: false, message: 'Credenciales incorrectas' });

    const user = rows[0];
    
    if (!user.password_hash) {
       return res.status(401).json({ ok: false, message: 'Este usuario semilla no tiene contraseña configurada' });
    }
    ...
    res.json({ ok: true, token, user: { nombre: user.nombre, correo: user.correo } });
  } catch (err) {
    console.error(err);
