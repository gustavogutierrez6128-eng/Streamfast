...
async function registrarUsuario(req, res) {
  ...
  try {
    ...
    await pool.query(
      'INSERT INTO usuarios (correo, password_hash, plan, nombre, fecha_registro) VALUES (?, ?, ?, ?, ?)',
      [emailFinal, passwordHash, 'basico', nombre, fechaRegistro]
    );
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    const [rows] = await pool.query('SELECT * FROM usuarios WHERE correo = ?', [emailFinal]);
    if (rows.length === 0) {
    ...
    const token = jwt.sign({ id: user.usuarioid, nombre: user.nombre }, JWT_SECRET, { expiresIn: '2h' });
    
    res.json({ 
      ok: true, 
      mensaje: 'Inicio de sesión correcto',
      token, 
      user: { nombre: user.nombre, email: user.correo, correo: user.correo },
      usuario: user
    });
  } catch (err) {
    console.error(err);
...
// Ruta general de series (Sincronizada)
...
// Ruta específica de la serie (Corregida a "actores" en español)
app.get('/api/series/:id', async (req, res) => {
  ...
  try {
    ...
    // Cambiado 'actors: actores' por 'actores: actores' para alinearse con Catalogo.html
    res.json({ ok: true, serie, episodios, actores: actores });
  } catch (err) {
    console.error(err);
